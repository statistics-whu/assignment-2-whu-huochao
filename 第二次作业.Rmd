...
a.
ggplot(kh_data,aes(x=kh_data$流失,y=kh_data$当月客户幸福指数,fill=kh_data$流失))+
  geom_boxplot()+
  theme_minimal()+
  labs(
...
ggplot(kh_data,aes(x=kh_data$流失,y=kh_data$坊间间隔变化,fill=kh_data$流失))+
  geom_boxplot()+
  theme_minimal()+
  labs(
    title = "坊间间隔变化在不同流失状态下的分布",
    x="流失状态",
    y="坊间间隔变化"
  )
